...
const MemberDetailsPopup = ({ member, onClose }: { member: TeamMember; onClose: () => void }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center overflow-auto p-4">
      <div className="bg-gray-900 border border-gray-400 p-6 md:p-6 rounded-lg shadow-md w-full max-w-lg md:max-w-2xl mx-auto my-8 md:my-20">
        <h2 className="text-xl md:text-2xl text-white font-bold mb-2 text-center">{member.name} - {member.role}</h2>
        <div className="overflow-auto max-h-[60vh]">
          <div className="mb-4 text-center">
            <img src={member.image} alt={member.name} className="mx-auto max-h-40 w-auto object-cover mb-4 border border-gray-500 transition-transform hover:scale-150 z-auto "/>
            <p className=" text-white  max-h-20">{member.details}</p>
          </div>
        </div>
        <button
          onClick={onClose}
...
export default function Team() {
  ...
  {teamMembers.map((member) => (
    <div key={member.name} className="w-full md:w-50 lg:w-80 mb-8 mx-auto overflow-hidden bg-transparent rounded-lg shadow-md border border-gray-500 relative">
      ...
      <div className="p-4 md:p-6 text-white  overflow-auto">
